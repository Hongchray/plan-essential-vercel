...
import { CheckIcon, XCircle, XIcon, ChevronDown, WandSparkles } from "lucide-react"
...
// RHF Props
// =====================
interface MultiSelectRHFProps extends Omit<VariantProps<typeof multiSelectVariants>, "badgeAnimation"> {
...
// =====================
export const MultiSelect = React.forwardRef<HTMLButtonElement, MultiSelectRHFProps>(
  ({
    options,
    form,
    name,
    ...
    const getOptionByValue = React.useCallback((val: string) => {
    ...
    const getBadgeAnimationClass = () => {
      if (!animationConfig?.badgeAnimation) return isAnimating ? "animate-bounce" : ""
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  {value.slice(0, maxCount).map(v => {
                    ...
                    return (
                      <Badge
                        key={v}
                        className={cn(getBadgeAnimationClass(), multiSelectVariants({ variant }))}
