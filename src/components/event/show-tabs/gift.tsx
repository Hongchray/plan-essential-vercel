...
export default function TabGift({guests, setGuests, expenses, setExpenses}: {guests: any[], setGuests: (guests: any[]) => void, expenses: any[], setExpenses: (expenses: any[]) => void}) {
  // Gift Component
  const [editingGift, setEditingGift] = useState(null);
    const [giftForm, setGiftForm] = useState({ item: '', value: '' });

    const updateGift = (guestId, giftData) => {
    ...
    const toggleReceived = (guestId) => {
      setGuests(guests.map(guest =>
        guest.id === guestId 
          ? { ...guest, gift: { ...guest.gift, received: !guest.gift.received } }
          : guest
      ));
    };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-green-50 p-4 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{receivedGifts}</div>
        ...
        <div className="space-y-3">
          {guests.map(guest => (
            <div key={guest.id} className="flex items-start justify-between p-4 border border-slate-200 rounded-lg">
                ...
                {editingGift === guest.id ? (
                  <div className="flex gap-2 mt-2">
                    <input
                      ...
                      value={giftForm.item}
                      onChange={(e) => setGiftForm({ ...giftForm, item: e.target.value })}
                  ...
                  <div className="text-sm text-slate-600">
                    {guest.gift.received ? (
                      <div>
                        ...
                        {guest.gift.value > 0 && ` ($${guest.gift.value.toFixed(2)})`}
                      </div>
                    ) : (
